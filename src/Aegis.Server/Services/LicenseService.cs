...
public class LicenseService(AegisDbContext dbContext)
{
    ...
    )
    {
        ...
        if (license.Status == LicenseStatus.Revoked)
            return new LicenseValidationResult(
                false,
                null,
                new LicenseValidationException("License Revoked.")
            );

        if (licenseFile != null)
        {
            var loadResult = await LicenseManager.LoadLicenseAsync(
                licenseFile,
            ...
            if (loadResult.Status != LicenseStatus.Valid)
                return new LicenseValidationResult(
                    ...
                    loadResult.Exception
                        ?? new LicenseValidationException(loadResult.Status.ToString())
                );
            ...
            if (
                loadedLicense.Type != license.Type
                ...
                return new LicenseValidationResult(
                    false,
                    null,
                    new LicenseValidationException(
                        "License file does not match the stored license."
                    )
                );
            ...
            switch (loadedLicense)
            {
                case NodeLockedLicense nl:
                    if (
                        ...
                        && validationParams.TryGetValue("HardwareId", out var hardwareId)
                ...
                case StandardLicense sl:
                    if (
                        ...
                        && (
                            ...
                            || (
                                validationParams.TryGetValue("UserName", out var userName)
                    ...
                    )
                    {
                        ...
                        return new LicenseValidationResult(
                            false,
                            null,
                            new UserMismatchException(message)
                        );
                    }
                ...
                case SubscriptionLicense subscriptionLicense:
                    if (
                        subscriptionLicense.SubscriptionStartDate.Add(
                            subscriptionLicense.SubscriptionDuration
                ...
                case FloatingLicense floatingLicense:
                    if (
                        floatingLicense.MaxActiveUsersCount != license.MaxActiveUsersCount
        ...
        switch (license.Type)
        {
            ...
            case LicenseType.Concurrent:
                ...
                try
                {
                    ...
                    var currentActiveUsers = await dbContext.Activations.CountAsync(a =>
                        a.LicenseId == license.LicenseId
                    );

                    if (currentActiveUsers < license.MaxActiveUsersCount)
                    {
                        dbContext.Activations.Add(
                            new Activation
                            {
            ...
            case LicenseType.Subscription:
                if (license.SubscriptionExpiryDate < DateTime.UtcNow)
                    return new LicenseActivationResult(
                        false,
                        new ExpiredLicenseException("Subscription has expired.")
                    );

                break;
            case LicenseType.Floating:
                var activeActivations = await dbContext.Activations.CountAsync(a =>
                    a.LicenseId == license.LicenseId
                );
            ...
            default:
                return new LicenseActivationResult(
                    false,
                    new InvalidLicenseFormatException("Invalid license type.")
                );
        }
    ...
    public async Task<LicenseDeactivationResult> DisconnectConcurrentLicenseUser(
        string licenseKey,
        string? hardwareId = null
    )
    {
    ...
    /// <returns>A <see cref="LicenseDeactivationResult" /> object containing the deactivation result.</returns>
    public async Task<LicenseDeactivationResult> RevokeLicenseAsync(
        string licenseKey,
        string? hardwareId = null
    )
    {
        var license = await dbContext
            .Licenses.Include(l => l.Activations)
            .FirstOrDefaultAsync(l => l.LicenseKey == licenseKey);
        ...
        switch (license.Type)
        {
            ...
            case LicenseType.Floating:
                ...
                try
                {
                    ...
                    var activation = license.Activations.FirstOrDefault(a =>
                        a.MachineId == hardwareId
                    );
                    if (activation != null)
                    {
                    ...
                    else
                    {
                        return new LicenseDeactivationResult(
                            false,
                            new NotFoundException("Activation not found.")
                        );
                    }
                }
                finally
    ...
    )
    {
        ...
        if (license.Type != LicenseType.Subscription)
            return new LicenseRenewalResult(
                false,
                "Invalid license type. Only subscription licenses can be renewed."
            );
        ...
        if (
            newExpirationDate < DateTime.UtcNow
            || newExpirationDate < license.SubscriptionExpiryDate
        )
            return new LicenseRenewalResult(
                false,
    ...
    {
        var activation = await dbContext.Activations.FirstOrDefaultAsync(a =>
            a.License.LicenseKey == licenseKey && a.MachineId == machineId
        );
    ...
    private License CreateLicenseEntity(LicenseGenerationRequest request)
    {
        return new License
        {
            ...
            SubscriptionExpiryDate =
                request.SubscriptionDuration != null
    ...
    private async Task AddFeaturesToLicenseAsync(License license, LicenseGenerationRequest request)
    {
        ...
        {
            var licenseFeature = await dbContext.LicenseFeatures.FirstOrDefaultAsync(lf =>
                lf.ProductId == request.ProductId && lf.FeatureId == featureData.Key
            );

            if (licenseFeature == null)
            {
                var product = await dbContext.Products.FirstAsync(p =>
                    p.ProductId == request.ProductId
                );
                var feature = await dbContext.Features.FirstAsync(f =>
                    f.FeatureId == featureData.Key
                );
    ...
    private byte[] GenerateLicenseFile(License license)
    {
        ...
        {
            LicenseType.Standard => LicenseManager.SaveLicense(
                new StandardLicense(baseLicense, license.IssuedTo)
            ),
            LicenseType.Trial => LicenseManager.SaveLicense(
                new TrialLicense(baseLicense, license.ExpirationDate!.Value - DateTime.UtcNow)
            ),
            LicenseType.NodeLocked => LicenseManager.SaveLicense(
                new NodeLockedLicense(baseLicense, license.HardwareId!)
            ),
            LicenseType.Subscription => LicenseManager.SaveLicense(
                new SubscriptionLicense(
                    baseLicense,
                    license.IssuedTo,
                    license.ExpirationDate!.Value - DateTime.UtcNow
                )
            ),
            LicenseType.Floating => LicenseManager.SaveLicense(
                new FloatingLicense(
                    baseLicense,
                    license.IssuedTo,
            ...
            LicenseType.Concurrent => LicenseManager.SaveLicense(
                new ConcurrentLicense(
                    baseLicense,
                    license.IssuedTo,
                    license.MaxActiveUsersCount!.Value
                )
    ...
    private BaseLicense MapLicenseToBaseLicense(License license)
    {
        return new BaseLicense
        {
            ...
            Features = license.LicenseFeatures.ToDictionary(
                lf => lf.Feature.FeatureName,
                lf => new Feature { Type = lf.Type, Data = lf.Data }
            ),
